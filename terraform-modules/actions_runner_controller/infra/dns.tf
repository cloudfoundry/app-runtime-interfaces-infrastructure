...
resource "google_dns_record_set" "arc_webhook_server_production" {
  ...
  name         = "${google_compute_global_address.arc_webhook_server_production[0].name}.${data.google_dns_managed_zone.dns.dns_name}"
  type         = "A"
  rrdatas      = [google_compute_global_address.arc_webhook_server_production[0].address]
  ttl          = 300
  project      = var.project
}
